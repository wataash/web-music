...
単曲の iReal HTMLから記載順のコード列・曲情報・注釈・元の表記を保持する譜面トークンを抽出する。Node.js >=22.5。
...
抽出結果は `{ format, title, artist, originalKey, chords, comments, annotations, score, unmappedSymbols }`。反復は展開しない。未知の品質は保持して unmappedSymbols に記録する。scramble(text) は50文字単位の自己逆変換で、符号化・復号に使える。暗号化ではない。
