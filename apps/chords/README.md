...
Use “☆ Favorite” to toggle the selected song. The library lists favorites first under “★ Favorites”, then every song under “All songs”, so a favorite appears in both; search and the playlist filter apply to both groups. Favorites work for both examples and imported charts. They are stored by song ID in localStorage (`chord-practice-favorites`) and survive “Reset settings and position”. Deleting and reimporting the same chart restores its favorite status.
