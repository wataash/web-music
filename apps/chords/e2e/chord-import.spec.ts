...
test("imports, searches, transposes, persists and deletes iReal songs", async ({ page }) => {
  ...
  const firstId = await page.getByLabel("Song", { exact: true }).inputValue();
  ...
  await page.getByLabel("Song", { exact: true }).selectOption({ label: "Import Two · Example" });
  const secondId = await page.getByLabel("Song", { exact: true }).inputValue();
  await expect(page.getByLabel("Song key", { exact: true })).toHaveValue("C#");
  await page.getByRole("button", { name: "Chord list", exact: true }).click();
  ...
  await page.reload();
  await expect(page.getByLabel("Song", { exact: true })).toHaveValue(secondId);
  await page.getByText("Import iReal Pro charts", { exact: true }).click();
  ...
  await expect(page.getByLabel("Song", { exact: true })).toHaveValue(firstId);
  await expect(page.getByLabel("Song", { exact: true }).locator('option').filter({ hasText: "Import One" })).toHaveCount(1);
  await page.getByRole("button", { name: "Delete selected imported chart" }).click();
  await expect(page.getByLabel("Song", { exact: true }).locator(`option[value="${firstId}"]`)).toHaveCount(0);
  await expect(page.getByLabel("Song", { exact: true }).locator(`option[value="${secondId}"]`)).toHaveCount(1);
});

test("invalid input leaves the current library usable", async ({ page }) => {
  await page.goto("/");
  await page.getByText("Import iReal Pro charts", { exact: true }).click();
  const selected = await page.getByLabel("Song", { exact: true }).inputValue();
  ...
  await expect(page.getByLabel("Song", { exact: true })).toHaveValue(selected);
});
...
test("imports and searches the complete local Jazz playlist", async ({ page }) => {
  ...
  await page.goto("/");
  const initial = await page.getByLabel("Song", { exact: true }).locator("option").count();
  await page.getByText("Import iReal Pro charts", { exact: true }).click();
  ...
  await expect(page.getByLabel("Song", { exact: true }).locator("option")).toHaveCount(initial + 1460);
  await page.getByLabel("Search songs", { exact: true }).fill(source.title);
  await page.getByLabel("Song", { exact: true }).selectOption({ label: `${source.title} · ${source.artist}` });
  await page.getByLabel("Song key", { exact: true }).selectOption("D");
