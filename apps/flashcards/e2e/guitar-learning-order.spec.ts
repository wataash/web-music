...
test("re-imports guitar priorities without resetting studied cards or review history", async ({ page }) => {
  ...
  const result = await page.evaluate(async () => {
    ...
    const stored = (await db.cards.toArray()).sort((a, b) => a.newOrder - b.newOrder);
    const seen = new Set<string>([studied.card.key]);
    const firstGroups = stored.filter(card => {
    ...
    return {
      ...
      firstKey: first.card.key, expectedFirstKeys: firstGroups.map(card => card.key),
      firstTags: first.note.tags, filteredOffset: filtered.note.fields[4],
  ...
  expect(result.expectedFirstKeys).toContain(result.firstKey);
  expect(result.firstTags).toContain("learning-level::1");
