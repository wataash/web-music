...
import { WEB_PACKAGE_SPEC } from "./package-spec";

const { values } = parseArgs({
  options: { output: { type: "string", short: "o" } },
});
const outputPath = resolve(values.output ?? "dist/intervals.json");
const artifacts = createWebDeckArtifacts();
const document = {
  format: "web-music-flashcards-deck",
  version: 1,
  deck: createWebPackage(WEB_PACKAGE_SPEC, artifacts.notes, artifacts.media),
} as const;
