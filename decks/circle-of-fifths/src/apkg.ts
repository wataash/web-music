...
export const MODEL_ID = 1_927_430_611;
export const INTERVALS_DECK_ID = 1_785_040_000_000;
export const FLAT3_DECK_ID = 1_785_040_000_001;
export const MAJOR3_DECK_ID = 1_785_040_000_002;
export const NOTE_TO_CELL_DECK_ID = 1_785_040_000_003;
export const OUTER_NOTE_TO_CELL_DECK_ID = 1_785_040_000_004;
export const INNER_NOTE_TO_CELL_DECK_ID = 1_785_040_000_005;
export const CELL_TO_NOTES_DECK_ID = 1_785_040_000_006;
export const OUTER_CELL_TO_NOTES_DECK_ID = 1_785_040_000_007;
export const INNER_CELL_TO_NOTES_DECK_ID = 1_785_040_000_008;
export const DECK_CONFIG_ID = 1_785_040_300_000;

const NOTE_ID_BASE = 1_785_040_100_000;
const CARD_ID_BASE = 1_785_040_200_000;
const TEMPLATE_ID = 1_847_201_003;
const TEMPLATE_NAME = "Card 1";
const DECK_CONFIG_NAME = `${ROOT_DECK_NAME} — Random New Cards`;
...
const DECK_DEFINITIONS: readonly DeckDefinition[] = [
  {
    id: NOTE_TO_CELL_DECK_ID,
    name: NOTE_TO_CELL_DECK_NAME,
    description: "Parent deck that studies both note-to-cell decks together.",
    hiddenByDefault: true,
  },
  {
    id: OUTER_NOTE_TO_CELL_DECK_ID,
    name: OUTER_NOTE_TO_CELL_DECK_NAME,
    description: "Answer the outer (major) cell for a note.",
    hiddenByDefault: true,
  },
  {
    id: INNER_NOTE_TO_CELL_DECK_ID,
    name: INNER_NOTE_TO_CELL_DECK_NAME,
    description: "Answer the inner (minor) cell for a note.",
    hiddenByDefault: true,
  },
  {
    id: CELL_TO_NOTES_DECK_ID,
    name: CELL_TO_NOTES_DECK_NAME,
    description: "Parent deck that studies both cell-to-notes decks together.",
    hiddenByDefault: true,
  },
  {
    id: OUTER_CELL_TO_NOTES_DECK_ID,
    name: OUTER_CELL_TO_NOTES_DECK_NAME,
    description: "Answer every note in an outer (major) cell.",
    hiddenByDefault: true,
  },
  {
    id: INNER_CELL_TO_NOTES_DECK_ID,
    name: INNER_CELL_TO_NOTES_DECK_NAME,
    description: "Answer every note in an inner (minor) cell.",
    hiddenByDefault: true,
  },
  {
    id: INTERVALS_DECK_ID,
    name: INTERVALS_DECK_NAME,
    description: "Parent deck that studies ♭3 and Δ3 together.",
    hiddenByDefault: true,
  },
  {
    id: FLAT3_DECK_ID,
    name: FLAT3_DECK_NAME,
    description: "Learn minor thirds with the circle of fifths.",
    hiddenByDefault: true,
  },
  {
    id: MAJOR3_DECK_ID,
    name: MAJOR3_DECK_NAME,
    description: "Learn major thirds with the circle of fifths.",
    hiddenByDefault: true,
  },
];
...
export async function writeAnkiPackage({
  ...
  const temporaryDirectory = await mkdtemp(
    join(tmpdir(), "circle-of-fifths-anki-"),
  );
...
export async function inspectAnkiPackage(
  ...
  const temporaryDirectory = await mkdtemp(
    join(tmpdir(), "circle-of-fifths-anki-inspect-"),
  );
...
function shuffledOrderKey(noteId: string): string {
  return createHash("sha256")
    .update(`circle-of-fifths:new-card-order:${noteId}`)
    .digest("hex");
}
...
function validatePackageInput(
  ...
  const noteGuids = new Set(notes.map(({ guid }) => guid));
  const mediaFilenames = new Set(media.map(({ filename }) => filename));

  if (noteIds.size !== notes.length) {
    throw new Error("duplicate note id");
  }
  if (noteGuids.size !== notes.length) {
    throw new Error("duplicate note guid");
  }
  if (mediaFilenames.size !== media.length) {
